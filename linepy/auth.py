...
class Auth(object):
    ...
    def getToken(self, mid):
        ...
        transport = THttpClient.THttpClient('https://gfs.line.naver.jp/api/v4/TalkService.do')
        ...
        getAccessKey = self.server.getJson('https://gfs.line.naver.jp/Q', allowHeader=True)

        transport = THttpClient.THttpClient('https://gfs.line.naver.jp/api/v4p/rs')
        transport.setCustomHeaders(headers)
        protocol = TCompactProtocol.TCompactProtocol(transport)
        _auth = AuthService.Client(protocol)
        transport.open()
        try:
            Ieq = Auth.loginRequests('1', {
               'keepLoggedIn': True,
               'systemName': None,
